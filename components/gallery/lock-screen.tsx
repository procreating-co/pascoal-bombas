...
export function LockScreen({ onUnlock }: { onUnlock: () => void }) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-black px-6 text-white">
      ...
      <h1 className="mt-6 text-balance text-center font-display text-4xl leading-[0.95] tracking-tight sm:text-5xl">Galeria de Conteúdos</h1>

      <form onSubmit={handleSubmit} className="mt-10 flex w-full max-w-sm flex-col items-center">
        ...
        {error && <span className="mt-3 font-mono text-xs uppercase tracking-wide text-red-400">Código incorreto. Tente novamente.</span>}
        <button
          type="submit"
          className="mt-10 inline-flex h-14 w-full items-center justify-center bg-white text-base font-medium text-black transition-transform hover:scale-[1.02]"
        >
          Entrar
        </button>
      </form>
